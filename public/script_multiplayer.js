...
player.on("up_movement", (room, changes) => {
  changes[color].removed.forEach((erase) => {
    piece_erase(erase);
  });

  piece_init(
    blue_turn ? [b_piece, b_dama] : [r_piece, r_dama],
    changes[color].added
  );
...
const game = () => {
  clean_piece_select();
  blue_turn ? set_ur_turn() : unset_ur_turn();
  ...
  if (win) {
    if (win == color) {
      ...
      document.querySelector(
        ".display p"
      ).innerText = `${nome}, não foi dessa vez. Você perdeu.`;
    }
