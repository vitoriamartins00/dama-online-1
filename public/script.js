...
async function handleLogin(email, senha) {
  ...
  if (response.status === 200) {
    ...
    window.location.replace("/perfil"); //verificar com o nicolas onde está a pagina do jogo//
...
if (formAtualizacao) {
  formAtualizacao.addEventListener("submit", async (e) => {
    ...
    const novoNomeusuario = document.getElementById("novoUsuario").value;
    ...
    const response = await fetch(formAtualizacao.action, {
      method: "POST",
      headers: {
        "content-type": "application/json",
      },
      body: JSON.stringify({ novoNomeusuario }),
      credentials: "include",
    });
    ...
    if (response.status === 200) {
      ...
      alert(`Falha na atualização: ${data.message}`);
    }
  });
}
...
if (deleteButton) {
  deleteButton.addEventListener("click", async () => {
    ...
    if (confirmar) {
      const response = await fetch("/api/auth/excluir-conta", {
        method: "POST",
        credentials: "include",
      });
      ...
      if (response.status === 200) {
        ...
        alert(`Falha ao excluir conts: ${data.message}`);
      }
    }
  });
}
