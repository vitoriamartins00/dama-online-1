  ...
  try {
    while (count < step_lim) {
      ...
      if (block.childNodes.length > 0) {
        ...
        if (block.childNodes.item(0).classList.contains(red ? "red" : "blue")) {
          block = null;
          break;
        }
        // ve se é inimigo e se for vai pular por cima dele
        if (
          block.childNodes.item(0).classList.contains(!red ? "red" : "blue")
        ) {
          if (ford) {
            if (left) {
              to_elim["NW"] = block;
...
const eliminate = (from, new_loc) => {
  ...
  if ((direction == "SW" || direction == "NW") && elim_coords[1] < new_loc[1]) {
    return false;
  }

  if ((direction == "SE" || direction == "NE") && elim_coords[1] > new_loc[1]) {
    return false;
  }
...
const show_turn = () => {
  document.getElementById("turno").innerText = blue_turn ? "Azul" : "Vermelho";
};
...
const select_piece = (id, elim, blue = true) => {
  ...
  if (elim) {
    ...
    if (back.length == 1) {
      ...
      if ((back_coords[0] == row + row_incre) & (back_coords[1] == col - 2)) {
        next = [...next, ...back];
      }
    }
...
const show_win = () => {
  ...
  if (winner) {
    document.querySelector(
      ".display p"
    ).innerText = `${winner}, parabéns!!! Você ganhou`;
