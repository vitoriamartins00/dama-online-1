...
const select_to_land = (
  ...
  try {
    while (count < step_lim) {
      ...
      if (!(0 <= row + step_y && row + step_y <= 7)) {
        throw new Error("a");
      }
      if (!(0 <= col + step_x && col + step_x <= 7)) {
        throw new Error("a");
      }
...
const piece_erase = (allies, enemies, idx, color) => {
  ...
  allies.pieces = allies.pieces.filter(
    (p) => !(p.coords[0] == idx[0] && p.coords[1] == idx[1])
  );
  allies.enemies.pieces = allies.enemies.pieces.filter(
    (p) => !(p.coords[0] == idx[0] && p.coords[1] == idx[1])
  );
  ...
  enemies.pieces = enemies.pieces.filter(
    (p) => !(p.coords[0] == 7 - idx[0] && p.coords[1] == 7 - idx[1])
  );
  enemies.enemies.pieces = e_pieces.filter(
    (p) => !(p.coords[0] == 7 - idx[0] && p.coords[1] == 7 - idx[1])
  );
...
const click_cheack_blanks = (match, select_piece, color) => {
  ...
  const selected = select_blank(
    allies,
    enemies,
    select_piece[0].coords,
    select_piece[0].dama,
    color
  );
...
const game = async (match, killed) => {
  ...
  if (match.red.enemies.pieces.length == 0) {
    ...
    await User.findByIdAndUpdate(untoken_id(match.red.token), {
      $inc: { partidas_vencidas: 1 },
    });
  }
  if (match.blue.enemies.pieces.length == 0) {
    match.win = "blue";
    await User.findByIdAndUpdate(untoken_id(match.blue.token), {
      $inc: { partidas_vencidas: 1 },
    });
  }
};
