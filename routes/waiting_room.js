...
const waiting_room = (io) => {
    io.of("/waiting").on("connection", async (socket) => {
        ...
        if (queue.length > 0) {
            ...
            if(s){
                await User.findByIdAndUpdate(untoken_id(p2_socket.data.token), {$inc: {partidas_jogadas:1}})
                await User.findByIdAndUpdate(untoken_id(token), {$inc: {partidas_jogadas:1}})
